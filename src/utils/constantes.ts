import type { Tom, TomTraducao, Provedor } from '../tipos';
...
export function idiomaCurto(codigo: string): string {
  const i = IDIOMAS.find((x) => x.codigo === codigo);
  return i ? i.curto : codigo.toUpperCase();
}

export const TONS: Array<{ valor: Tom; rotulo: string; emoji: string }> = [
...
export const TONS_TRADUCAO: Array<{ valor: TomTraducao; rotulo: string; emoji: string }> = [
...
export const URLS_CHAVES: Record<Exclude<Provedor, 'ollama'>, string> = {
